...
import { ConfigurationError, ERR_DESCRIPTION } from "../errors/errors.js";
...
export class PKCEUtils {
  ...
  static generateCodeVerifier(config?: PKCECodeVerifierConfig): string {
    ...
    if (length < 43 || length > 128) {
      throw new ConfigurationError(
        ERR_DESCRIPTION.PKCE.INVALID_CODE_VERIFIER_LENGTH,
      );
    }
  ...
  static async generateCodeChallenge(codeVerifier: string): Promise<string> {
    if (!codeVerifier) {
      throw new ConfigurationError(ERR_DESCRIPTION.PKCE.INVALID_CODE_VERIFIER);
    }
